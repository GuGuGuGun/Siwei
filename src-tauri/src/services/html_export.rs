...
.title{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px;font-weight:700}.actions{display:flex;align-items:center;gap:8px}.seg{display:flex;gap:2px;border:1px solid var(--line);border-radius:8px;background:#f3efe6;padding:2px}.seg button,.icon{height:32px;border:0;border-radius:6px;background:transparent;color:#52525b;cursor:pointer}.seg button{padding:0 10px;font-size:12px;font-weight:700}.seg button.active{background:white;color:#18181b;box-shadow:0 1px 3px rgba(0,0,0,.08)}.icon{border:1px solid var(--line);background:var(--panel);padding:0 10px;font-size:12px;font-weight:700}
.stage{min-height:0;flex:1;overflow:hidden}.view{display:none;height:100%}.view.active{display:block}.outline{height:100%;overflow:auto;padding:32px}.outline-inner{max-width:920px;margin:0 auto}.outline h1{margin:0 0 28px;border-bottom:1px dashed var(--line);padding-bottom:16px;font-size:30px}.node{margin:8px 0}.node-card{border:1px solid var(--line);border-radius:10px;background:rgba(255,255,255,.74);padding:10px 12px;box-shadow:0 8px 22px rgba(0,0,0,.035)}.node-row{display:flex;align-items:center;gap:8px}.collapse{width:24px;height:24px;border:0;border-radius:6px;background:#f4eadc;color:var(--accent);cursor:pointer}.collapse:disabled{opacity:.24;cursor:default}.note{margin:6px 0 0 32px;color:#52525b;font-size:13px;white-space:pre-wrap}.tags{color:var(--muted);font-size:12px}.task{font-family:ui-monospace,Consolas,monospace;color:#047857}.children{margin-left:28px}
.mindmap{position:relative;height:100%;overflow:hidden}.canvas{height:100%;width:100%;touch-action:none;cursor:grab}.canvas:active{cursor:grabbing}.edge{fill:none;stroke:#a27b5c;stroke-width:1.8;stroke-dasharray:5 5}.map-node rect{fill:#faf6ec;stroke:rgba(139,90,43,.3);stroke-width:2;stroke-dasharray:5 5;filter:drop-shadow(0 8px 16px rgba(0,0,0,.08))}.map-node text{fill:#3f3f46;font-size:12px;font-weight:700}.hint{position:absolute;bottom:14px;left:14px;border:1px solid var(--line);border-radius:8px;background:rgba(255,255,255,.72);padding:6px 8px;color:#71717a;font-size:12px}
...
(() => {
  ...
  function renderShell() {
    rootEl.innerHTML = `
      <header class="bar">
        ...
        <div class="actions">
          <div class="seg">
            <button type="button" data-action="toggle-view" data-view="outline">大纲</button>
            <button type="button" data-action="toggle-view" data-view="mindmap">导图</button>
          </div>
  ...
  function renderOutlineNodes(nodes) {
    return nodes.map((node) => {
      ...
      return `<div class="node"><div class="node-card"><div class="node-row"><button type="button" class="collapse" data-action="toggle-collapse" data-node-id="${escapeText(node.id)}" ${children.length ? '' : 'disabled'}>${children.length ? (isCollapsed ? '›' : '⌄') : ''}</button><div>${task}${escapeText(node.text || '空白节点')}${tags}</div></div>${note}</div>${!isCollapsed && children.length ? `<div class="children">${renderOutlineNodes(children)}</div>` : ''}</div>`;
    }).join('');
  }
  ...
  function collectVisible(node, depth = 0, indexByDepth = new Map(), result = { nodes: [], edges: [] }) {
    ...
    result.nodes.push({ node, depth, x: depth * 250, y: index * 90 });
  ...
  function renderMindMap() {
    ...
    const edges = graph.edges.map((edge) => {
      ...
      return `<path class="edge" d="M${source.x + 180} ${source.y + 30} C${source.x + 220} ${source.y + 30}, ${target.x - 40} ${target.y + 30}, ${target.x} ${target.y + 30}" />`;
    }).join('');
    const nodes = graph.nodes.map(({ node, x, y }) => {
      const label = escapeText(node.text || '空白节点');
      const width = Math.max(170, Math.min(260, 72 + label.length * 12));
      return `<g class="map-node" transform="translate(${x},${y})"><rect width="${width}" height="60" rx="12"></rect><text x="14" y="35">${label}</text>${(node.children || []).length ? `<text x="14" y="52" style="font-size:10px;fill:#8a5a2b">${(node.children || []).length} 个子节点</text>` : ''}</g>`;
    }).join('');
